...
class MinifyHelper extends AppHelper {
    ...
    private function _path($assets, $type) {
        ...
        $timestampEnabled = $assetTimestamp === 'force' || ($assetTimestamp === true && Configure::read('debug') > 0);
		Configure::write('Asset.timestamp', false);

        $files = array();
        foreach ($assets as $asset) {
            $p = substr($this->assetUrl($asset, $options), 1);
            if ($timestampEnabled) {
                $p .= '&' . @filemtime(WWW_ROOT . str_replace('/', DS, $asset));
            }
            array_push($files, $p);
        }
